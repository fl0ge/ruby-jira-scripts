# Ruby script to remove a fix version by name to all issues using a JQL Search query
...
def remove_fix_version(http, login, password, fix_version, issue_id)
	remove_fix_version_request = Net::HTTP::Put.new("/rest/api/2/issue/#{issue_id}")
	remove_fix_version_request.basic_auth "#{login}", "#{password}"
	remove_fix_version_request.content_type = 'application/json'
	remove_fix_version_request.body = '{ "update" : { "fixVersions": [	{"remove":	{"name": "'+"#{fix_version}"+'" }}]}}'

	remove_fix_version_response = http.request(remove_fix_version_request)
end
...
for issue in issues
	...
	puts "Removing fixVersion #{fix_version} to issue #{jira_issue_key} (#{issue['fields']['summary']})"
	remove_fix_version http, login, password, fix_version, issue_id
